    ...
    return (
        <div className="title">
            ...
            <div className="expert">
                ...
                <Button icon="mail" onClick={props.onLeaveMessage} >留言</Button>
            </div>
        </div>
    )
}
...
const PricePanel = (props: PriceProps) => {
    const methods: CounselingMethod[] = [{
        id: 'audio',
        name: '线上',
        price: 0// props.counselor ? props.counselor.audioPrice : 0
    }, {
        id: 'video',
        name: '线下',
        price: 0 //props.counselor ? props.counselor.videoPrice : 0
    }
        // , {
        //     id: 'ftf',
        //     name: '面对面',
    ...
    return (
        <div className="price-panel">
            ...
            <Radio.Group value={method} buttonStyle="solid" onChange={e => props.onSetMethod(methods.find(m => m.id === e.target.value))} >
                {
                    methods.map(m =>
                        <Radio.Button key={m.id} value={m.id} disabled={m.price === -1}>{`${m.name} ￥${m.price}/次`}</Radio.Button>
                    )
                }
            </Radio.Group>
...
class AppointMntModal extends React.Component<Props, IAppointMntModalState> {
    ...
    handlePayment = () => {
        ...
        // handle payment
        message.loading('预约中，请稍后', 2.5).then(() => {
            // post record
            this.props.dispatch(fetchAction('operation/appoint', { data }))
            this.closeModal()
        }, () => {
            message.error('预约失败，请稍后重试', 1)
        })
    }
    ...
    render() {
        ...
        const Footer = (
            <div className="footer">
                ...
                <Button type="primary" onClick={this.handlePayment} >发起预约</Button>
            </div>
        )
        return (
            <Modal
